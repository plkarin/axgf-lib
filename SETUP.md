...
> Ubuntu 24 LTS quick-start and a self-contained test battery covering every
> public feature of the library. Keep this file as your regression baseline:
...
Create the test runner anywhere on the machine:

```bash
mkdir -p ~/axgf-e2e/src
cd ~/axgf-e2e
```
...
[dependencies]
axgf-rs    = { path = "/home/cbrain/axgf-lib" }  # adjust path if needed
serde_json = "1"
base64     = "0.22"
```

### 3.2 Test runner - src/main.rs
...
use axgf_rs::{
    ...
    export_bundle, import_bundle, inspect, update_entity, validate, DeletePolicy,
    EntityKind,
};
...
fn diag_codes(env: &Value) -> Vec<&str> {
    env["diagnostics"]
        .as_array()
        .unwrap_or(&vec![])
        .iter()
...
fn test_04_add_person(bundle: &str) -> (String, String) {
    ...
    check("T04 bundle contains 1 person",
        persons.len() == 1,
        "person count mismatch");
    let (id, _) = persons.iter().next().unwrap();
    let person_id = id.clone();
    (data(&env).to_string(), person_id)
}

fn test_05_add_uuid_generated(bundle: &str) -> String {
    // Entity without id -> library generates UUID v4
    let person = json!({
        "type": "person",
        "axgf_version": "1.0",
        "identity": {
    ...
    let b: Value = serde_json::from_str(&data(&env).to_string()).unwrap();
    let persons = b["persons"].as_object().unwrap();
    check("T05 bundle now has 2 persons",
        persons.len() == 2,
        "person count mismatch");
    // Confirm the auto-generated id looks like a UUID v4
    let ids: Vec<&str> = persons.keys().map(|k| k.as_str()).collect();
    let all_uuid = ids.iter().all(|id| {
...
fn test_08_add_link(bundle: &str, jean_id: &str) -> String {
    // Add a third person (Jules - godson) then link Jean as godfather
    let jules = json!({
        "type": "person",
        "axgf_version": "1.0",
        "identity": {
...
fn test_11_update_entity(bundle: &str, jean_id: &str) -> String {
    // Retrieve Jean's current record and patch the bio
    let b: Value = serde_json::from_str(bundle).unwrap();
    let mut jean = b["persons"][jean_id].clone();
    jean["id"] = json!(jean_id);
    jean["bio"] = json!("Schoolteacher, village school founder. Decorated with the Order of Merit in 1972.");
    ...
    check("T11 bio was updated",
        b2["persons"][jean_id]["bio"].as_str()
            .map(|s| s.contains("Merite")) == Some(true),
        "bio not updated");
    data(&env).to_string()
}
...
fn test_14_delete_cascade_policy(bundle: &str, jean_id: &str) -> String {
    ...
    check("T14 Jean removed from persons",
        ...
        "Jean still present");
    // Family must still exist but Jean's entry must be gone from union.persons
    let families = b["families"].as_object().unwrap();
    let no_jean_in_families = families.values().all(|f| {
        f["union"]["persons"].as_array()
            .unwrap_or(&vec![])
            .iter()
            .all(|p| p["person_id"].as_str() != Some(jean_id))
    });
    check("T14 Jean removed from all family unions under Cascade",
        no_jean_in_families,
...
fn test_15_validate_full_bundle(bundle: &str) {
    ...
    let warnings: Vec<_> = env["diagnostics"].as_array()
        .unwrap_or(&vec![])
        .iter()
        .filter(|d| d["severity"].as_str() == Some("error"))
        .collect();
    check("T15 no error-severity diagnostics",
        warnings.is_empty(),
...
fn test_16_unsupported_spec_version() {
    ...
    check("T16 unknown spec version is rejected",
        !ok(&env),
        "expected error, got ok");
    check("T16 diagnostic is UNSUPPORTED_SPEC_VERSION",
        diag_codes(&env).contains(&"UNSUPPORTED_SPEC_VERSION"),
        "wrong diagnostic code");
}

fn test_17_export_and_reimport(bundle: &str) {
    // flat JSON -> ZIP -> flat JSON: stats must be preserved
    ...
    check("T17 import_bundle of exported ZIP returns ok",
        ok(&env_imp),
        "status != ok");
...
fn test_18_deduplicate() {
    // Build a bundle with two identical-spouse families
    let bundle0 = data(&parse(&create_bundle(None).to_json())).to_string();

    let alice = json!({"type":"person","axgf_version":"1.0","id":"aaaaaaaa-0000-4000-8000-000000000001","identity":{"name":{"display":"Alice","components":[{"type":"given_name","value":"Alice","order":1}]},"gender":{"value":"F"},"is_living":false,"visibility":"members"}});
    let bob   = json!({"type":"person","axgf_version":"1.0","id":"bbbbbbbb-0000-4000-8000-000000000002","identity":{"name":{"display":"Bob","components":[{"type":"given_name","value":"Bob","order":1}]},"gender":{"value":"M"},"is_living":false,"visibility":"members"}});

    let b1 = data(&parse(&add_entity(&bundle0, EntityKind::Person, &alice.to_string()).to_json())).to_string();
    let b2 = data(&parse(&add_entity(&b1, EntityKind::Person, &bob.to_string()).to_json())).to_string();

    let fam_a = json!({"type":"family","axgf_version":"1.0","id":"faaaaaaa-0000-4000-8000-000000000001","union":{"type":"marriage","persons":[{"person_id":"aaaaaaaa-0000-4000-8000-000000000001","role":"spouse"},{"person_id":"bbbbbbbb-0000-4000-8000-000000000002","role":"spouse"}],"confidence":0.9},"children":[]});
    let fam_b = json!({"type":"family","axgf_version":"1.0","id":"fbbbbbbb-0000-4000-8000-000000000002","union":{"type":"marriage","persons":[{"person_id":"aaaaaaaa-0000-4000-8000-000000000001","role":"spouse"},{"person_id":"bbbbbbbb-0000-4000-8000-000000000002","role":"spouse"}],"confidence":0.9},"children":[]});
...
fn test_19_gedcom_conversion() {
    // Minimal inline GEDCOM 5.5.1 covering: partial date, circa, name alias,
    // OCCU, NOTE. Real-file test (tree2-fixed.ged) is optional and
...
0 TRLR\n";
    ...
    check("T19 persons == 2",
        ...
        "person count mismatch");
    check("T19 families == 1 (+ 1 marriage event)",
        b["families"].as_object().map(|m| m.len()) == Some(1),
        "family count mismatch");
    ...
    check("T19 places >= 1 (Saint-Denis deduplicated)",
        b["places"].as_object().map(|m| m.len()).unwrap_or(0) >= 1,
        "no places");

    // Circa date: Elise's birth ABT 1925 must set circa=true
    ...
    check("T19 converted bundle passes validation",
        ok(&env_v),
        "converted bundle invalid");

    // Optional: large real-world file
    let real_ged_path = "/home/cbrain/axgf-tools/tree2-fixed.ged";
    if std::path::Path::new(real_ged_path).exists() {
        let bytes = std::fs::read(real_ged_path).unwrap();
        let env_r = parse(&convert_gedcom(&bytes, 0.8, "en").to_json());
        check("T19b real-world tree2-fixed.ged converts ok",
            ok(&env_r),
        ...
        println!("[SKIPPED] T19b real-world GEDCOM (file not found)");
    }
}

fn test_20_forward_compatibility() {
    // An entity with an unknown field must survive a round-trip without loss
    let person_with_future_field = json!({
    ...
    let env = parse(&add_entity(&bundle0, EntityKind::Person,
                                &person_with_future_field.to_string()).to_json());
...
fn main() {
    ...
    // Retrieve Elise's auto-generated id
    ...
    let (b07, _event_id) = test_07_add_event(&b06, &jean_id, &elise_id, &family_id);
...
[PASSED] T19 families == 1 (+ 1 marriage event)
[PASSED] T19 events >= 1 (marriage event created)
[PASSED] T19 occupations == 1
[PASSED] T19 places >= 1 (Saint-Denis deduplicated)
[PASSED] T19 ABT date produces circa=true
[PASSED] T19 converted bundle passes validation
[PASSED] T19b real-world tree2-fixed.ged converts ok     (if file present)
[PASSED] T19b real-world: persons >= 760                 (if file present)
...
cd ~/axgf-e2e && cargo run

# Or wire into a shell script:
cargo run --manifest-path ~/axgf-e2e/Cargo.toml \
  && echo "regression: ok" \
...
# WASM target (requires wasm-pack or wasm32 toolchain)
...
All functions return a uniform `Envelope`: `{ status, data, diagnostics[] }`.
